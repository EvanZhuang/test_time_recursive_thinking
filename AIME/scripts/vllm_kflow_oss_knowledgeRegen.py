...
def main():
    ...
    sampling_params = SamplingParams(
        ...
        max_tokens=int(0.8 * args.max_new_tokens),
...
[Things to Avoid]: A standalone list of max 10 answers to avoid, note down what was not working for you, which answer is definitely wrong and why. Write it concisely in a standalone manner.
    ...
    for reflex_ctr in range(args.reflex_size):
        ...
        for example_ctr, example in enumerate(dataset):
            ...
            if reflex_ctr == 0:
                ...
                # Process previous generation to extract mistakes and summary
                ...
                # Extract and store knowledge about mistakes to avoid
                new_knowledge = x.split("[Things to Avoid]:")[-1].split("[Summary]")[0].strip()
                new_knowledge = " ".join(fill_to_length(sent_tokenize(new_knowledge), max_length=int(0.1 * args.max_new_tokens), tokenizer=tokenizer))
            ...
            if reflex_ctr == 0:
                tmp_conv = [
                    {"role": "system", "content": "PLACEHOLDER"},
                    {"role": "user", 'content': f"{question}" + init_prompt}
                ]
                if reflex_ctr > 0:
                    print("Problem {}, {} | Still not working".format(example_ctr, gold_answer))
            else:
                # Build mistake list for refinement prompt
                ...
                if msg['role'] == 'system':
                    harmony_messages.append(
                        Message.from_role_and_content(
                            Role.SYSTEM, 
                            SystemContent.new().with_reasoning_effort(REASONING_EFFORT["high"])
                        )
                    )
                elif msg['role'] == 'user':
        ...
        for example_ctr, (out, gold) in enumerate(zip(outputs, gold_answers)):
            # For OAI models, parse the completion tokens back into structured messages
            output_tokens = out.outputs[0].token_ids
            #entries = encoding.parse_messages_from_completion_tokens(output_tokens, Role.ASSISTANT)
            generation = out.outputs[0].text  # Also get the raw text
            ...
            # Extract and track predicted answer
